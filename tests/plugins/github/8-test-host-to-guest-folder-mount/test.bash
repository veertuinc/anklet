...
# Written on the handler under /tmp before dispatch; guest sees this tree under
...
cleanup() {
    ...
    echo "] Removing host mount marker from handler /tmp..."
    ssh_to_host "handler-8-16" "rm -f /tmp/${HOST_MOUNT_MARKER_FILENAME}" 2>/dev/null || true
...
echo "] Creating marker file on handler /tmp/${HOST_MOUNT_MARKER_FILENAME} (must appear inside guest via mount)..."
ssh_to_host "handler-8-16" "rm -f /tmp/${HOST_MOUNT_MARKER_FILENAME} && printf '%s' '${HOST_MOUNT_MARKER_BODY}' > /tmp/${HOST_MOUNT_MARKER_FILENAME}"
