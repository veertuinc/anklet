...
func InQueue(pluginCtx context.Context, logger *slog.Logger, jobID int64, queue string) (bool, error) {
	...
	for _, queueItem := range queued {
		workflowJobEvent, err, typeErr := database.UnwrapPayload[internalGithub.SimplifiedWorkflowJobEvent](queueItem)
		if err != nil {
		...
		case *github.WorkflowJobEvent:
			...
			if *workflowJob.Action == "queued" {
				if exists_in_array_partial(simplifiedWorkflowJobEvent.WorkflowJob.Labels, []string{"anka-template"}) {
					...
					inQueue, err := InQueue(pluginCtx, logger, *simplifiedWorkflowJobEvent.WorkflowJob.ID, "anklet/jobs/github/queued/"+pluginConfig.Owner)
					if err != nil {
					...
					if !inQueue { // if it doesn't exist already
						...
						wrappedJobPayload := map[string]any{
							...
							"payload": simplifiedWorkflowJobEvent,
				...
				if exists_in_array_partial(simplifiedWorkflowJobEvent.WorkflowJob.Labels, []string{"anka-template"}) {
					// make sure it doesn't already exist
					inQueue, err := InQueue(pluginCtx, logger, *simplifiedWorkflowJobEvent.WorkflowJob.ID, "anklet/jobs/github/in_progress/"+pluginConfig.Owner)
					if err != nil {
						logger.ErrorContext(pluginCtx, "error searching in queue", "error", err)
						return
					}
				...
				if exists_in_array_partial(simplifiedWorkflowJobEvent.WorkflowJob.Labels, []string{"anka-template"}) {
					...
					for _, queue := range queues {
						...
						go func(queue string) {
							defer wg.Done()
							inQueue, err := InQueue(pluginCtx, logger, *simplifiedWorkflowJobEvent.WorkflowJob.ID, queue)
							if err != nil {
					...
					if inAQueue { // only add completed if it's in a queue
						inCompletedQueue, err := InQueue(pluginCtx, logger, *simplifiedWorkflowJobEvent.WorkflowJob.ID, "anklet/jobs/github/completed/"+pluginConfig.Owner)
						if err != nil {
							logger.ErrorContext(pluginCtx, "error searching in queue", "error", err)
							return
						}
...
MainLoop:
	for i := len(toRedeliver) - 1; i >= 0; i-- { // make sure we process/redeliver queued before completed
		...
		var workflowJobEventPayload internalGithub.SimplifiedWorkflowJobEvent
		...
		if *hookDelivery.Action == "queued" {
			...
			for _, otherHookDelivery := range *hookDeliveries {
				if *otherHookDelivery.Action == "completed" &&
					...
					var otherWorkflowJobEventPayload internalGithub.SimplifiedWorkflowJobEvent
