...
func CheckForCompletedJobs(
	...
	for {
		...
		// get the job ID
		existingJob, err := databaseContainer.Client.LIndex(pluginCtx, pluginQueueName, 0).Result()
...
func cleanup(
	...
	for {
		...
		if cleaningJobJSON == "" {
			...
			_, err = databaseContainer.Client.RPopLPush(
				cleanupContext,
				pluginQueueName,
	...
	if queuedJob.Action != "paused" { // no need to do this, we already did it when we got the paused job
		...
		if err != nil {
			...
			for {
				...
				fmt.Println("pausedQueueJob", pausedQueueJob)
