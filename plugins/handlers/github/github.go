...
func cleanup(
	...
	if !pluginGlobals.FirstCleanupComplete {
		...
		// determine if the job is already running or not, and if it is, we can't clean it or send it back
		...
		if queuedJobString != "" {
			...
			logger.DebugContext(pluginCtx, "workflowJob", "workflowJob", workflowJob)
