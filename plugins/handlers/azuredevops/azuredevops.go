	...
	for {
		...
		case "anka.VM":
			...
			if strings.ToUpper(os.Getenv("LOG_LEVEL")) == "DEBUG" || strings.ToUpper(os.Getenv("LOG_LEVEL")) == "DEV" {
				err = ankaCLI.AnkaCopyOutOfVM(cleanupContext, queuedJob.AnkaVM.Name, "/Users/anka/azp-agent/_diag", "/tmp/"+queuedJob.AnkaVM.Name)
				if err != nil {
					logging.Warn(pluginCtx, "error copying azure devops agent out of vm", "error", err)
				}
			}
...
func Run(
	...
	logging.Info(pluginCtx, "starting azure devops plugin")
	...
	mainQueueName := "anklet/jobs/azuredevops/queued/" + pluginConfig.Owner
	mainCompletedQueueName := "anklet/jobs/azuredevops/completed/" + pluginConfig.Owner
	// hash tag needed for avoiding "CROSSSLOT Keys in request don't hash to the same slot"
	pluginQueueName := "anklet/jobs/azuredevops/queued/" + pluginConfig.Owner + "/" + "{" + pluginConfig.Name + "}"
	pluginCompletedQueueName := "anklet/jobs/azuredevops/completed/" + pluginConfig.Owner + "/" + pluginConfig.Name
	mainInProgressQueueName := "anklet/jobs/azuredevops/in_progress/" + pluginConfig.Owner
	pausedQueueName := "anklet/jobs/azuredevops/paused/" + pluginConfig.Owner
	...
	if err != nil && err != redis.Nil {
		// logging.Error(pluginCtx, "error getting last object from anklet/jobs/azuredevops/queued/"+pluginConfig.Owner+"/"+pluginConfig.Name, "error", err)
	...
	if queuedJobString != "" {
		...
		for {
			...
			defer func() {
				...
				if pausedQueuedJobString != "" {
					...
					if parseErr == nil && typeErr == nil && pausedJob.WorkflowJob.ID != nil {
						...
						currentJobJSON, err := internalGithub.GetJobJSONFromQueueByID(pluginCtx, *pausedJob.WorkflowJob.ID, "anklet/jobs/azuredevops/queued/"+pluginConfig.Owner+"/{"+pausedJob.PausedOn+"}")
			...
			originalHostJob, err := internalGithub.GetJobFromQueueByKeyAndValue(
				pluginCtx,
				"anklet/jobs/azuredevops/queued/"+pluginConfig.Owner+"/{"+pausedQueuedJob.PausedOn+"}",
				"workflow_job.id",
			...
			_, err = databaseContainer.RetryLRem(pluginCtx, "anklet/jobs/azuredevops/queued/"+pluginConfig.Owner+"/{"+pausedQueuedJob.PausedOn+"}", 1, originalHostJob)
			if err != nil {
		...
		// queuedJobString, err = databaseContainer.Client.LIndex(pluginCtx, "anklet/jobs/azuredevops/queued/"+pluginConfig.Owner, globals.QueueIndex).Result()
		// if err == nil {
		// 	// Remove the job at the specific index
		// 	_, err = databaseContainer.Client.LSet(pluginCtx, "anklet/jobs/azuredevops/queued/"+pluginConfig.Owner, globals.QueueIndex, "TO_BE_REMOVED").Result()
		// 	if err != nil {
		// 		return pluginCtx, fmt.Errorf("error setting job at index to TO_BE_REMOVED: %s", err.Error())
		// 	}
		// 	_, err = databaseContainer.Client.LRem(pluginCtx, "anklet/jobs/azuredevops/queued/"+pluginConfig.Owner, 1, "TO_BE_REMOVED").Result()
		// 	if err != nil {
		...
		// 	return pluginCtx, fmt.Errorf("error getting queued jobs: %s", err.Error())
		// }
		// databaseContainer.Client.RPush(pluginCtx, "anklet/jobs/azuredevops/queued/"+pluginConfig.Owner+"/"+pluginConfig.Name, queuedJobString)
	}
	...
	// a job in anklet/jobs/azuredevops/queued and also a anklet/jobs/azuredevops/completed
	...
	installRunnerPath := filepath.Join(workerGlobals.PluginsPath, "handlers", "azuredevops", "install-runner.bash")
	registerRunnerPath := filepath.Join(workerGlobals.PluginsPath, "handlers", "azuredevops", "register-runner.bash")
	startRunnerPath := filepath.Join(workerGlobals.PluginsPath, "handlers", "azuredevops", "start-runner.bash")
	...
	if installRunnerErr != nil || registerRunnerErr != nil || startRunnerErr != nil {
		...
		return pluginCtx, fmt.Errorf("must include install-runner.bash, register-runner.bash, and start-runner.bash in %s/handlers/azuredevops/", workerGlobals.PluginsPath)
	}
	...
	// Install runner
	logging.Info(pluginCtx, "installing azure devops agent inside of vm")
	...
	logging.Info(pluginCtx, "registering azure devops agent inside of vm", "queuedJob", queuedJob)
	...
	logging.Info(pluginCtx, "starting azure devops agent inside of vm")
	...
	logging.Info(pluginCtx, "finished preparing anka VM with azure devops agent")
	queuedJob.Action = "in_progress"
	err, completed = internalGithub.UpdateJobInDB(pluginCtx, pluginQueueName, &queuedJob)
	if completed != nil {
		pluginGlobals.JobChannel <- internalGithub.QueueJob{Action: "finish"}
