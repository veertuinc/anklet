    ...
    | jq -Rr '
        ...
        try (fromjson |
            if type == "object" then
                ...
                ((.workflowJobRunID // (if ($attrs | type) == "object" then $attrs.workflowJobRunID else null end) // "") | tostring) as $runID |
                ((.workflowJobID // (if ($attrs | type) == "object" then $attrs.workflowJobID else null end) // "") | tostring) as $jobID |
                ((.queue // "") | tostring) as $queue |
                (if ($runID | length) > 0 then " workflowJobRunID=" + $runID else "" end) as $workflowJobRunIDStr |
                (if ($jobID | length) > 0 then " workflowJobID=" + $jobID else "" end) as $workflowJobIDStr |
                (if ($queue | length) > 0 then " queue=" + $queue else "" end) as $queueStr |
                ...
                (if $levelUpper == "ERROR" then
                    "\u001b[31mtime=" + $timeStr + " name=" + $coloredName + "\u001b[31m level=" + $levelStr + " msg=" + $msgStr + $workflowJobRunIDStr + $workflowJobIDStr + $queueStr + $errorStr + " attributes=" + $attrsJson + "\u001b[0m"
                elif ($levelUpper == "WARN" or $levelUpper == "WARNING") then
                    "\u001b[33mtime=" + $timeStr + " name=" + $coloredName + "\u001b[33m level=" + $levelStr + " msg=" + $msgStr + $workflowJobRunIDStr + $workflowJobIDStr + $queueStr + $errorStr + " attributes=" + $attrsJson + "\u001b[0m"
                elif $levelUpper == "DEBUG" then
                    "\u001b[90mtime=" + $timeStr + " name=" + $coloredName + "\u001b[90m level=" + $levelStr + " msg=" + $msgStr + $workflowJobRunIDStr + $workflowJobIDStr + $queueStr + $errorStr + " attributes=" + $attrsJson + "\u001b[0m"
                else
                    "\u001b[33mtime\u001b[0m=\u001b[37m" + $timeStr + "\u001b[0m name=" + $coloredName + " level=" + $levelStr + " msg=" + $msgStr + $workflowJobRunIDStr + $workflowJobIDStr + $queueStr + $errorStr + " attributes=" + $attrsJson
                end)
            else
                $raw
            end
